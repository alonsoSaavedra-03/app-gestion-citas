...
<div class="container mt-5">
    <div class="form-card">
        ...
        <h4 class="mb-4">Editar Tratamiento</h4>

        <form action="/tratamientos/{{ $tratamiento->id }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="mb-3">
                ...
                <input type="text" name="duracion" class="form-control" value="{{ $tratamiento->duracion }}">
            </div>
            ...
            <div class="mb-3">
                ...
                <select name="diagnostico_id" class="form-control">
                    @foreach ($diagnosticos as $d)
                        <option value="{{ $d->id }}"
                            {{ $d->id == $tratamiento->diagnostico_id ? 'selected' : '' }}>
                            {{ $d->descripcion }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                ...
                <select name="medico_id" class="form-control">
                    @foreach ($medicos as $m)
                        <option value="{{ $m->id }}"
                            {{ $m->id == $tratamiento->medico_id ? 'selected' : '' }}>
                            {{ $m->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3">
                <label>Estado</label>
                <input type="text" name="estado" class="form-control" value="{{ $tratamiento->estado }}">
            </div>

            <div class="mb-3">
                <label>Frecuencia</label>
                <input type="text" name="frecuencia_administracion" class="form-control"
                    value="{{ $tratamiento->frecuencia_administracion }}">
            </div>
